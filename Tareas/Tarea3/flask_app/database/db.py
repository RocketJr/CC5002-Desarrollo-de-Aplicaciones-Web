...
def insert_pedido(tipo, descripcion, comuna, nombre_comprador, email_comprador, celular_comprador):
    conn = get_conn()
    cursor = conn.cursor()
    comuna_id = get_comuna_id(comuna)
...
def get_pedidos_paginated(page, page_size):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT id, tipo, descripcion, comuna_id, nombre_comprador, email_comprador, celular_comprador FROM pedido ORDER BY id DESC LIMIT %s, %s;", ((page - 1) * page_size, page_size))
    productos = cursor.fetchall()
    return productos
...
def insert_pedido_verdura_fruta(nombre, pedido_id):
    conn = get_conn()
    cursor = conn.cursor()
    tipo_verdura_fruta_id = get_producto_verdura_fruta_id(nombre)
...
def get_tipos_asociados_pedido(pedido_id):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT TVF.nombre FROM tipo_verdura_fruta TVF, pedido_verdura_fruta PVF WHERE TVF.id=PVF.tipo_verdura_fruta_id AND PVF.pedido_id=%s;", (pedido_id,))
    tipos = cursor.fetchall()
    return [tipo[0] for tipo in tipos]
...
def get_pedido_por_ids(id):
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT tipo, descripcion, comuna_id, nombre_comprador, email_comprador, celular_comprador FROM pedido WHERE id=%s;", (id,))
    nombre = cursor.fetchone()
    return nombre

def register_productor(tipo, descripcion, comuna, nombre_productor, email_productor, celular_productor):
...
def register_comprador(tipo, descripcion, comuna, nombre_comprador, email_comprador, celular_comprador):
    pedido_id = insert_pedido(tipo, descripcion, comuna, nombre_comprador, email_comprador, celular_comprador)
    return True, None, pedido_id

# -- querys para las estadisticas --
def stats_productos():
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT TVF.nombre, COUNT(*) FROM producto_verdura_fruta PVF, tipo_verdura_fruta TVF WHERE PVF.tipo_verdura_fruta_id = TVF.id GROUP BY TVF.nombre;")
    productos = cursor.fetchall()
    return productos
...
def stats_pedidos():
    conn = get_conn()
    cursor = conn.cursor()
    cursor.execute("SELECT CO.nombre, COUNT(*) FROM pedido PE, comuna CO WHERE PE.comuna_id=CO.id GROUP BY PE.comuna_id;")

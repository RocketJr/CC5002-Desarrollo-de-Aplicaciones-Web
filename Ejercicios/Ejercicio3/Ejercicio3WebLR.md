...
**Respuesta**: El Server Side Template Injection es una vulneravilidad que ocurre cuando un atacante, puede inyectar código malicioso en una plantilla que se ejecuta en el servidor. Esta vulnerabilidad se puede encontrar en tecnologías como Jinja.
...
def validar_input(mi_input):
  malas = ["garfield"]
  return bool(re.search(r"\d", mi_input)) and 5 <= len(mi_input) <= 30 and not mi_input in malas 

@app.route('/pregunta', method=('GET', 'POST'))
def pregunta():
  if request.method == 'POST':
    input_usuario = request.form.get("pregunta")
